...
class SEDMLWriter {
    ...
    org.jdom.Element getXML(Simulation sedmlSim) {
        ...
        if (sedmlSim.getSimulationKind().equals(SEDMLTags.SIMUL_UTC_KIND)) {
            // various attributes depending on kind
        ...
        // FSKSimulation
        else if (sedmlSim.getSimulationKind().equals(SEDMLTags.SIMUL_FSK_KIND)) {
            node = new Element(SEDMLTags.SIM_FSK);
            node.setAttribute(SEDMLTags.SIM_ATTR_ID, sedmlSim.getId());

            String name = sedmlSim.getName();
            if (name != null)
                node.setAttribute(SEDMLTags.SIM_ATTR_NAME, name);

            FskSimulation fskSim = (FskSimulation) sedmlSim;

            String language = fskSim.getLanguage();
            if (language != null)
                node.setAttribute(SEDMLTags.SIM_ATTR_LANGUAGE, language);

            node.setAttribute(SEDMLTags.SIM_ATTR_TYPE, fskSim.getType());

            return node;  // return node to skip the inclusion of algorithms
    ...
    private void addNotesAndAnnotation(SEDBase sedbase, Element node) {
        ...
        Annotation ann = sedbase.getAnnotation();
        if (ann != null) {
    ...
    org.jdom.Element getXML(Output sedmlOutput) {
        ...
        } else if (sedmlOutput.getKind().equals(SEDMLTags.SCRIPT_KIND)) {
            node = new Element(SEDMLTags.OUTPUT_SCRIPT);

            String id = sedmlOutput.getId();
            if (id != null)
                node.setAttribute(SEDMLTags.OUTPUT_ID, id);

            String name = sedmlOutput.getName();
            if (name != null)
                node.setAttribute(SEDMLTags.OUTPUT_NAME, name);

            Script script = (Script) sedmlOutput;

            node.setAttribute(SEDMLTags.OUTPUT_TYPE, script.getType());

            if (script.isEmbedded()) {
                node.setText(script.getScript());
            } else {
                node.setAttribute(SEDMLTags.OUTPUT_SRC, script.getScript());
            }
        }
