...
class SchemaValidatorImpl {
    ...
    private String getSchema(String xmlAsString) throws JDOMException,
        ...
        // make sure this is sedml and not sbml or something
        if (!sedRoot.getName ().equals ("sedML"))
            throw new IllegalArgumentException ("Not a SED-ML file");

        // make sure the root node contains level/version attributes
        if (sedRoot.getAttribute("level") == null || sedRoot.getAttribute("version") == null)
            throw new IllegalArgumentException ("Invalid SED-ML file");
        ...
        if (level.equals("1") && version.equals("1")) {
            ...
            throw new IllegalArgumentException(
                    "Invalid level/version combination - must be 1-1 or 1-2 but was "
                            + level + "," + version);
        }
    }
