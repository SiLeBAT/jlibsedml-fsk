...
public class AttributeUniqenessAnalyser {
     ...
     * an element amongst its siblings. If there is more than 1 such attribute
     * found, preference will be given to an attribue with name 'id' ( case
     * insensitive) else the first suitable attribute will be returned.
     * 
     * @param el
     * @return An {@link AttDataObj}, or <code>null</code> if no such attribute
     *         could be found.
     */
    public AttDataObj getUniqueAttributeForElement(Element el) {
        List<Element> sibs = getSiblings(el);
        Iterator<Element> it = sibs.iterator();
        ...
        while (it.hasNext()) {
            ...
            for (Attribute att : atts) {
                ...
                if (el.getAttribute(att.getName(),
    ...
    /**
     * Gets the index of an element amongst its siblings. This method is
     * namespace aware and thus the index will be for elements with the same
     * element name and namespace. If this element cannot be found, the method
     * returns -1.
     * 
     * @param el
     *            A non-null JDom Element that is part of a Document (i.e., not
     *            standalone ).
    ...
    public int getIndexForElementAmongstSiblings(Element el) {
        ...
        for (int i = 0; i < sibs.size(); i++) {
            ...
            if (sib.getNamespace() != null
                    && sib.getNamespace().equals(el.getNamespace())
